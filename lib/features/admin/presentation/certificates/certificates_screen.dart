...
class _CertificatesScreenState extends State<CertificatesScreen> {
  ...
  Future<void> _generateCertificate() async {
    ...
    try {
      ...
      final String issue = _formatDate(_form.date!);

      await firestore.collection('certificates').add({
        'userId': _form.userId,
        'userName': _form.user,
        'role': _form.role.toLowerCase(), // store "student"/"teacher"
        'certName': _form.certName,
        'issueDate': issue,
        'issueDateTimestamp': Timestamp.fromDate(_form.date!),
      ...
      showDialog(
        ...
        builder: (_) => SuccessFailureDialog(
          ...
          onPressed: () {
            Navigator.pop(context);
            context.pushNamed('generatedCertificates');
          },
        ),
      );
    } catch (e) {
              ...
              child: Column(
                children: [
                  ...
                  _buildButton(
                    ...
                    bgColor: AppColors.secondaryDark,
                    textColor: AppColors.textPrimary,
                    onTap: () => context.pushNamed('generatedCertificates'),
                  ),
                ],
              ),
            ),
