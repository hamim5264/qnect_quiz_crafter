...
final teacherCourseFormControllerProvider =
    NotifierProvider<TeacherCourseFormController, TeacherCourseFormState>(
...
class TeacherCourseFormController extends Notifier<TeacherCourseFormState> {
  ...
  void updatePrice(String v) =>
      state = state.copyWith(price: int.tryParse(v) ?? 0, errorMessage: null);
  ...
  void updateDiscount(String v) {
    if (v == 'Free') {
      ...
      state = state.copyWith(discountPercent: p, errorMessage: null);
    }
  }
